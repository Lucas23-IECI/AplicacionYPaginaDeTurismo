...
export default function EventDetailPage() {
  ...
  usePageTitle(event?.title ?? 'Cargando...');
  ...
  const handleTwitter = () => {
    const url = encodeURIComponent(window.location.href);
    const text = encodeURIComponent(event ? `${event.title} — Explora Chile` : '');
                ...
                <div className="relative flex-1">
                  ...
                  {showShare && (
                    <div className="absolute top-full mt-2 right-0 bg-white border border-stone-200 rounded-xl shadow-lg p-2 min-w-[180px] z-30">
                      ...
                      <button onClick={handleTwitter} className="w-full text-left px-3 py-2 text-sm text-stone-700 hover:bg-stone-50 rounded-lg flex items-center gap-2">
                        <svg viewBox="0 0 24 24" className="w-4 h-4 fill-stone-800"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                        X (Twitter)
                      </button>
                      <button onClick={handleFacebook} className="w-full text-left px-3 py-2 text-sm text-stone-700 hover:bg-stone-50 rounded-lg flex items-center gap-2">
